/**
 * Telegram channel — Grammy bot with voice pipeline (Groq STT + OpenAI TTS).
 *
...
export function startTelegram(deps: TelegramDeps): { stop: () => Promise<void> } {
  const { config } = deps;
  ...
  const transcribe = async (audioBuffer: Buffer): Promise<string | null> => {
    const groqKey = config.voice.groqApiKey;
  ...
  bot.catch((err) => console.error("[telegram] Grammy error:", err.error ?? err));
  ...
  bot.on("message:text", async (ctx) => {
    const senderId = String(ctx.from?.id);
    if (!isAllowed(senderId)) return;
    if (processedMessages.has(ctx.message.message_id)) return;
    processedMessages.add(ctx.message.message_id);
  ...
  bot.on("message:voice", async (ctx) => {
    ...
    if (!config.voice.groqApiKey) { await ctx.reply("Voice transcription is not configured."); return; }
    ...
    try {
      ...
      if (audioBuffer) {
        await ctx.replyWithVoice(new Blob([audioBuffer], { type: "audio/mpeg" }) as any);
      }
    } catch (err) {
  ...
  // --- Start polling ---
  bot.start({ onStart: () => console.log("[telegram] Bot is running") });
