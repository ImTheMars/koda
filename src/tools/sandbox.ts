/**
 * Safe Sandbox — runs code in an isolated Docker container.
 *
 * Per-task containers with hard resource limits:
 *   --memory 512m  --cpus 0.5  --network none  --read-only /  --rm
 *
 * Falls back gracefully to exec if Docker is unavailable.
...
export async function registerSandboxTools(deps: { workspace: string }): Promise<ToolSet> {
  ...
  if (!dockerAvailable) {
    console.log("[boot] Sandbox: Docker not available — runSandboxed unavailable");
    return {};
  }
  ...
  const runSandboxed = tool({
    ...
    inputSchema: z.object({
      ...
      image: z.string().optional().describe("Docker image (default: alpine:latest)"),
      memory: z.string().optional().describe("Memory limit, e.g. '512m', '1g' (default: 512m)"),
      cpus: z.string().optional().describe("CPU limit, e.g. '0.5', '1' (default: 0.5)"),
